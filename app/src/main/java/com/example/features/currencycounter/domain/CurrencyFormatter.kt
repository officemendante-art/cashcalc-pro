...
class CurrencyFormatter {
    ...
    fun formatRupee(amount: Double): String {
        ...
        val formattedInt = applyIndianGrouping(wholePart.toString())
    ...
    /**
     * Finds all numeric parts in an expression string and applies Indian numeral grouping to their whole parts.
     */
    fun formatExpression(expression: String): String {
        val numberRegex = Regex("\\d+(?:\\.\\d+)?")
        return numberRegex.replace(expression) { matchResult ->
            val fullMatch = matchResult.value
            val pointIndex = fullMatch.indexOf('.')
            
            val integerPart = if (pointIndex == -1) fullMatch else fullMatch.substring(0, pointIndex)
            val fractionalPart = if (pointIndex == -1) "" else fullMatch.substring(pointIndex)
            
            val formattedIntegerPart = applyIndianGrouping(integerPart)
            formattedIntegerPart + fractionalPart
        }
    }

    private fun applyIndianGrouping(numStr: String): String {
